...
def compute_physics_loss(model, domain_points, density=1.0, viscosity=0.01, gravity=(0, -9.81)):
    ...
    if torch.isnan(continuity_loss) or torch.isnan(momentum_x_loss) or torch.isnan(momentum_y_loss):
        print("Warning: NaN detected in physics loss")
        print(f"continuity_loss: {continuity_loss}")
        print(f"momentum_x_loss: {momentum_x_loss}")
        print(f"momentum_y_loss: {momentum_y_loss}")
    
    # Print average magnitudes of terms to verify gravity is significant
    with torch.no_grad():
        print(f"Average magnitudes in y-momentum equation:")
        print(f"  Advection term: {torch.mean(torch.abs(density * (u * v_x + v * v_y))):.6f}")
        print(f"  Pressure gradient: {torch.mean(torch.abs(p_y)):.6f}")
        print(f"  Viscosity term: {torch.mean(torch.abs(viscosity * (v_xx + v_yy))):.6f}")
        print(f"  Gravity term: {torch.mean(torch.abs(density * g[1])):.6f}")
    
    # Total physics loss - give higher weight to y-momentum to emphasize gravity
    physics_loss = continuity_loss + momentum_x_loss + 2.0 * momentum_y_loss
